...
namespace Core_MVC.Controllers
{
    public class Rookies : Controller
    {
        ...
        public Rookies(IPersonService personService)
        {
        ...
        public IActionResult Create(Person member)
        {
            ...
            {
                var addMember = _personService.Create(member);
        ...
        public IActionResult Edit(Person member, string email)
        {
            if (ModelState.IsValid)
            {
                var editMember = _personService.Edit(member, email);
        ...
        public IActionResult Delete(string email)
        {
            if (ModelState.IsValid)
            {
                var delMember = _personService.Delete(email); 
                var member = _personService.Detail(email); 
                HttpContext.Session.SetString("Person", JsonConvert.SerializeObject(member));
                ViewBag.Message = string.Format("Deleted person {1} {2}", member.FirstName, member.LastName);
                return this.View(delMember);
            }
            return RedirectToAction(nameof(Index));
